...
public class ItemDAOImpl implements ItemDAO {
	...
	public List<ItemBean> getAllItem() {
		...
		ibList = template.query(sql, new Object[] { itid, startRecordNo, pageSize },
	...
	public List<ItemValBean> getItemVal(int itemId) {
		...
		ivbList = template.query(sql, new Object[] { itemId },
	...
	public void updateItem(ItemBean ib, int beforeItemId) {
		...
		try {
			template.update(sql,
					new Object[] { ib.getItemID(), ib.getItemHeader(),
							new SerialClob(GlobalService.StringToCharArray(ib.getItemDes())), ib.getItemPrice(),
							ib.getItId(), ib.getItemdiscount(), ib.getItemPic1(), ib.getItemPic2(), ib.getItemPic3(),
							ib.getItemPic4(), ib.getItemPic5(), ib.getItemstatusid(), beforeItemId });
		...
		// ps.setClob(3, new
		// SerialClob(GlobalService.StringToCharArray(ib.getItemDes())));
	...
	public void setItemValBean(List<ItemValBean> ibvList, int id) {
		String sql = "insert into item_val (Itemid,itemSerialNumber,Itemcolor,Itemsize,Itemqty ) "
				+ " values(?,?,?,?,?) ";

		for (ItemValBean ibv : ibvList) {
			template.update(sql, new Object[] { id, ibv.getItemSerialNumber(), ibv.getItemColor(), ibv.getItemSize(),
					ibv.getItemQty() });
		}
	...
	// @Override
	// public void setItemValBean(List<ItemValBean> ibvList, int id) {
	// String sql = "insert into item_val
	// (Itemid,itemSerialNumber,Itemcolor,Itemsize,Itemqty ) " + "
	// values(?,?,?,?,?) ";
	// Connection con = null;
	// PreparedStatement ps=null;
	// try {
		...
		try {
			template.update(sql,
					new Object[] { ib.getItemID(), ib.getItemHeader(),
							new SerialClob(GlobalService.StringToCharArray(ib.getItemDes())), ib.getItemPrice(),
							ib.getItId(), ib.getItemdiscount(), ib.getItemPic1(), ib.getItemPic2(), ib.getItemPic3(),
							ib.getItemPic4(), ib.getItemPic5(), ib.getItemstatusid() });
		} catch (DataAccessException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (SerialException e) {
	...
	public List<FeedBackBean> getItemFeedBack(Integer itemId) {
		String sql = "Select * from feedback where itemId = ?";
		List<FeedBackBean> feedBackList = new ArrayList<>();
		feedBackList = template.query(sql, new Object[] { itemId },
	...
	public void insertCP(CouponBean cb) {
		...
		template.update(sql ,  new Object[]{cb.getCpId() , cb.getCpDes() , cb.getCpVal() ,cb.getCpQty() , cb.getValid()
				,cb.getInvalid() , cb.getmId()}); 
	}
